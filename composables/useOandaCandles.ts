...
export const useOandaCandles = (
  ...
  const candles = computed(() => {
    ...
    if (
      livePrice &&
      livePrice.value &&
      livePrice.value.bids?.[0]?.price &&
      livePrice.value.asks?.[0]?.price
    ) {
  ...
  const calculateIchimoku = (): {
    ...
    const senkouSpanB =
      (Math.max(...senkouHighs) + Math.min(...senkouLows)) / 2;
  ...
  const recognizePatterns = (): CandlestickPattern[] => {
    ...
    const upperShadow = (candle: any) =>
      candle.high - Math.max(candle.open, candle.close);
    const lowerShadow = (candle: any) =>
  ...
  const analyzeMarketStructure = (): MarketStructure | null => {
    ...
    if (shortTermSMA && longTermSMA) {
      if (shortTermSMA > longTermSMA) {
        ...
        strength = Math.min(
          100,
          50 + ((shortTermSMA - longTermSMA) / longTermSMA) * 1000
        );
      } else if (shortTermSMA < longTermSMA) {
        trend = "downtrend";
        strength = Math.min(
          100,
          50 + ((longTermSMA - shortTermSMA) / longTermSMA) * 1000
        );
      }
    }
    ...
    const support = [
      ...new Set(
        recentLows.filter((low) => {
          const count = recentLows.filter(
            (l) => Math.abs(l - low) < low * 0.001
          ).length;
          return count >= 2;
        })
      ),
    ...
    const resistance = [
      ...new Set(
        recentHighs.filter((high) => {
          const count = recentHighs.filter(
            (h) => Math.abs(h - high) < high * 0.001
          ).length;
          return count >= 2;
        })
      ),

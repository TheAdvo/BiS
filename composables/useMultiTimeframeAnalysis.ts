...
export interface UseMultiTimeframeAnalysisReturn {
  ...
  activeTimeframe: any;
  timeframes: any;
  currentTimeframeData: any;
  fetchAllTimeframes: () => Promise<void>;
  fetchTimeframeData: (timeframe: any) => Promise<void>;
  startAutoRefresh: () => void;
  calculateOverallBias: () => void;
}
...
export const useMultiTimeframeAnalysis = (instrument: string = "EUR_USD") => {
  ...
  const calculateOverallBias = () => {
    ...
    const trends = [
      analysis.value.M5.trend,
      analysis.value.H1.trend,
      analysis.value.H4.trend,
      analysis.value.D.trend,
    ];

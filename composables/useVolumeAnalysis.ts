...
export const useVolumeAnalysis = () => {
  ...
  const calculateOBV = (candles: OandaCandle[]): number | null => {
    ...
    for (let i = 1; i < candles.length; i++) {
      ...
      if (current.complete && previous.complete && current.mid && previous.mid && current.volume) {
  ...
  const calculateMFI = (candles: OandaCandle[], period: number = 14): number | null => {
  ...
  const generateVolumeProfile = (candles: OandaCandle[], levels: number = 20): VolumeProfile[] => {
    ...
    validCandles.forEach(candle => {
      ...
      for (let price = low; price <= high; price += priceStep) {
        ...
        volumeByLevel[levelPrice] = (volumeByLevel[levelPrice] || 0) + volumePerLevel
      }
    })
    ...
    const profile: VolumeProfile[] = Object.entries(volumeByLevel)
      .map(([price, volume]) => ({
        ...
        type: volume > maxVolumeAtLevel * 0.7 ? 'high' as const :
              volume > maxVolumeAtLevel * 0.3 ? 'medium' as const : 'low' as const
      }))
  ...
  const analyzePriceVolumeRelationship = (candles: OandaCandle[]): 'bullish' | 'bearish' | 'neutral' => {
    ...
    for (let i = 1; i < recentCandles.length; i++) {
      ...
      if (current.complete && previous.complete && current.mid && previous.mid && current.volume && previous.volume) {
        const priceChange = parseFloat(current.mid.c) - parseFloat(previous.mid.c)
  ...
  const generateVolumeAlerts = (candles: OandaCandle[]): VolumeAlert[] => {
    ...
    if (currentVolume > avgVolume * 2) {
      alerts.push({
        ...
        description: `Volume spike: ${Math.round((currentVolume / avgVolume) * 100)}% above average`,
    ...
    if (currentVolume > avgVolume * 1.5) {
      const priceChange = recent.length > 1 ?
        currentPrice - parseFloat(recent[recent.length - 2].mid!.c) : 0

      if (Math.abs(priceChange) > 0.001) { // Significant price movement
        alerts.push({
          ...
          price: currentPrice,
          volume: currentVolume,
          description: `Volume spike with ${priceChange > 0 ? 'bullish' : 'bearish'} price action`,
          significance: 'high'
        })
      }
    }
  ...
  const calculateVolumeIndicators = (candles: OandaCandle[]) => {
    ...
    try {
      ...
      const volumeMA = candles.length >= 20 ?
        candles.slice(-20)
          .filter(c => c.complete && c.volume)
          .reduce((sum, c) => sum + c.volume!, 0) / 20 : null

      // Current volume ratio
      const currentVolume = candles[candles.length - 1]?.volume
      const volumeRatio = volumeMA && currentVolume ? currentVolume / volumeMA : null
  ...
  const volumeStrength = computed(() => {
    const { volumeRatio, mfi, priceVolumeConfirmation } = volumeIndicators.value
  ...
  const valueArea = computed(() => {
    ...
    const totalVolume = volumeProfile.value.reduce((sum, level) => sum + level.volume, 0)
    ...
    const prices = valueAreaLevels.map(l => l.priceLevel).sort((a, b) => a - b)

...
export const useTradingSignals = () => {
  ...
  const generateSignals = async (instrument: string, candles: OandaCandle[], timeframe: string = 'M5') => {
      ...
      if (bollinger) {
        ...
        if (latestPrice <= lower && percentB < 0.1) {
          newSignals.push(createSignal({
            ...
            confidence: 80,
            price: latestPrice,
            reason: `Price touching lower Bollinger Band (${percentB.toFixed(2)}%B)`,
          ...
          newSignals.push(createSignal({
            ...
            confidence: 80,
            price: latestPrice,
            reason: `Price touching upper Bollinger Band (${percentB.toFixed(2)}%B)`,
            indicators: ['Bollinger Bands'],
            timeframe
          }))
        }
    ...
    const confluenceSignals = findConfluenceSignals(newSignals, latestPrice, timeframe)

    // Filter by minimum confidence
    const filteredSignals = [...newSignals, ...confluenceSignals]
      .filter(signal => signal.confidence >= configuration.value.minConfidence)
  ...
  const createSignal = (params: Partial<TradingSignal> & { type: 'buy' | 'sell', price: number, reason: string, timeframe: string }): TradingSignal => {
    ...
    return {
      ...
      suggestedStopLoss: params.type === 'buy'
        ? params.price - stopLossDistance
        : params.price + stopLossDistance,
      suggestedTakeProfit: params.type === 'buy'
  ...
  const findConfluenceSignals = (signals: TradingSignal[], price: number, timeframe: string): TradingSignal[] => {
    ...
    if (buySignals.length >= 2) {
      const combinedIndicators = [...new Set(buySignals.flatMap(s => s.indicators))]
      const avgConfidence = buySignals.reduce((sum, s) => sum + s.confidence, 0) / buySignals.length

      confluenceSignals.push(createSignal({
        type: 'buy',
        strength: 'strong',
    ...
    if (sellSignals.length >= 2) {
      const combinedIndicators = [...new Set(sellSignals.flatMap(s => s.indicators))]
  ...
  const addSignal = (signal: TradingSignal) => {
    ...
    const todaySignals = signals.value.filter(s => s.timestamp.toDateString() === today)
  ...
  const updateSignalStatus = (signalId: string, status: TradingSignal['status']) => {
  ...
  const signalStats = computed(() => {
    ...
    const triggered = signals.value.filter(s => s.status === 'triggered').length
  ...
  const expireOldSignals = () => {
    ...
    signals.value.forEach(signal => {
      if (signal.status === 'active') {
        ...
        const maxAge = signal.timeframe === 'M5' ? 2 : signal.timeframe === 'H1' ? 12 : 24
